...
class TestCourseModel:
    ...
    def test_create_course_by_teacher(self, teacher_account):
        """Only managers/admins with add_course perm can add courses"""
        with pytest.raises(PermissionDenied):
            Course.objects.create(**self.data, assigned_by=teacher_account)

    def test_create_course_by_manager(self, manager_account):
        """Only manager/admins with add_course perm can add courses"""
        with pytest.raises(PermissionDenied):
            Course.objects.create(**self.data, assigned_by=manager_account)
    ...
    def test_get_days_display(self, accessed_manager_account):
        ...
        assert course.get_days_display == [
            WeekDays.SATURDAY.label,
            WeekDays.SUNDAY.label,
        ]

    def test_delete_with_status_completed(self, accessed_manager_account):
