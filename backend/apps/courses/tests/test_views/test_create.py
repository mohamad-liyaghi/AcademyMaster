...
class TestCreateCourseView:
    ...
    def test_create_by_manager_no_perm(self, api_client, manager_account):
        assert not manager_account.has_perm(perm_object=Course.get_permission("add"))
        api_client.force_authenticate(manager_account)
        response = api_client.post(self.url_path, self.data, format="json")
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_create_and_set_assigned_by(self, api_client, accessed_manager_account):
        api_client.force_authenticate(accessed_manager_account)
        response = api_client.post(self.url_path, self.data, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        assert (
            Course.objects.get(token=response.data["token"]).assigned_by
            == accessed_manager_account
        )
