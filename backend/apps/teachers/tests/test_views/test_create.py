...
class TestTeacherCreateView:
    ...
    def test_create_by_admin(self, api_client, superuser, another_account):
        self.data["user"] = another_account.token
        api_client.force_authenticate(superuser)
        response = api_client.post(self.url_path, self.data)

        assert response.status_code == status.HTTP_201_CREATED
        assert another_account.is_teacher()
        assert another_account.teacher.promoted_by == superuser
    ...
    def test_create_by_accessed_manager(self, api_client, accessed_manager_account):
    ...
    def test_promote_manager(self, api_client, manager_account, superuser):
    ...
    def test_promote_teacher_twice(self, api_client, teacher_account, superuser):
