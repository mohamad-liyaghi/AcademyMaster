...
class TestTeacherDeleteView:
    ...
    def setup(self, active_account):
        self.teacher = Teacher.objects.create(user=active_account)
        self.teacher_account = active_account
        self.url_name = "teachers:delete_teacher"
        self.url_path = reverse(
            self.url_name, kwargs={"teacher_token": self.teacher.token}
        )
    ...
    def test_delete_by_admin(self, api_client, superuser):
        ...
        self.teacher_account.refresh_from_db()
        assert not self.teacher_account.is_teacher()
    ...
    def test_delete_by_manager_no_permission(self, api_client, manager_account):
