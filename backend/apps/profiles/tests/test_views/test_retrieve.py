...
class TestRetrieveProfileView:
    ...
    def test_retrieve_profile_unauthorized(self, superuser, api_client):
        url = reverse(self.url_path, kwargs={"profile_token": superuser.profile.token})
    ...
    def test_retrieve_others_via_student(self, superuser, active_account, api_client):
    ...
    def test_student_get_own_profile(self, active_account, api_client):
        ...
        url = reverse(self.url_path, kwargs={"profile_token": profile.token})
        response = api_client.get(url)
        assert response.status_code == status.HTTP_200_OK

    def test_get_non_exist_profile(self, superuser, api_client, unique_uuid):
        api_client.force_authenticate(user=superuser)
        url = reverse(self.url_path, kwargs={"profile_token": unique_uuid})
        response = api_client.get(url)
