...
class TestCreateEnrollmentView:
    ...
    def test_create_by_student(self, api_client, active_account):
        ...
        assert response.data["user"] == str(active_account)
    ...
    def test_create_twice_pending_enrollment(self, api_client, active_account):
        ...
        api_client.force_authenticate(active_account)
        api_client.post(self.url_path, self.data, format="json")
        response = api_client.post(self.url_path, self.data, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_create_already_enrolled(self, api_client, create_enrollment):
    ...
    def test_create_already_failed(self, api_client, active_account, create_enrollment):
