...
class TestManagerModel:
    ...
    def test_promote_by_non_accessed_fails(self, manager_account, active_account):
        """Only Admin/Manager with add_manager perm can promote others"""
        assert not manager_account.has_perm(perm_object=Manager.get_permission("add"))
        with pytest.raises(PermissionDenied):
            Manager.objects.create(user=active_account, promoted_by=manager_account)
    ...
    def test_promote_manager_by_admin(self, superuser, active_account):
        Manager.objects.create(user=active_account, promoted_by=superuser)
        assert active_account.is_manager()
    ...
    def test_remove_permission(self, manager_account):
        ...
        assert manager_account.has_perm(perm_object=Manager.get_permission("add"))

        manager_account.user_permissions.remove(Manager.get_permission("add"))
        assert not manager_account.has_perm(perm_object=Manager.get_permission("add"))

    def test_remove_permission_after_deletion(self, accessed_manager_account):
        """Remove a user permission when its manager instance gets deleted"""
        assert accessed_manager_account.has_perm(
            perm_object=Manager.get_permission("add")
        )
    ...
    def test_create_manager_with_permission(self, active_account):
        Manager.objects.create_with_permissions(
            user=active_account, permissions=[Manager.get_permission("add")]
        )
        assert active_account.has_perm(perm_object=Manager.get_permission("add"))
        assert active_account.has_perm(perm_object=Manager.get_permission("add"))
