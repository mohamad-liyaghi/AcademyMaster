...
class TestManagerDeleteView:
    ...
    def test_delete_unauthorized_fails(self, api_client):
    ...
    def test_delete_by_accessed_manager(self, api_client, accessed_manager_account):
        """
        Manager with managers.delete_manager perm can delete other managers
        """
    ...
    def test_delete_by_manager_without_permission(self, api_client):
        ...
        assert not self.manager.has_perm(perm_object=Manager.get_permission("delete"))
        resp = api_client.delete(self.url_path)
        assert resp.status_code == status.HTTP_403_FORBIDDEN

    def test_delete_non_existing_manager(self, api_client, superuser, unique_uuid):
        api_client.force_authenticate(superuser)
        url_path = reverse(self.url_name, kwargs={"manager_token": unique_uuid})
